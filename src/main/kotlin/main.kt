...
fun main() {
        ...
        while(true) {
            ...
            val comission = getComission(cardType, totalTransfers + transferValue, transferValue)
...
fun checkLimits(cardType: Int = 3, totalTransfers: Int, transferValue: Int): Boolean {
    return when {
        cardType < 3 && transferValue <= CARD_DAY_LIMIT && (totalTransfers) <= CARD_MONTH_LIMIT -> true
        cardType == 3 && transferValue <= VK_DAY_LIMIT && (totalTransfers) <= VK_MONTH_LIMIT -> true
        else -> false
    }
}
...
fun getComission(cardType: Int = 3, totalTransfers: Int, transferValue: Int): Int {
    return when(cardType) {
        1 -> getMaestroComission(totalTransfers, transferValue)
